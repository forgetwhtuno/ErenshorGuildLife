...
namespace ErenshorGuildLife
{
    ...
    {
        internal const float Width = 136f;
        ...
        internal void Initialize(float storedX, float storedY, Action<float, float> persist, Action toggle)
        {
            Dispose();
            _toggle = toggle;
            _root = RetainedUiKit.CreateCanvas("ErenshorGuildLauncherCanvas", 510);
            RectTransform canvas = _root.GetComponent<RectTransform>();
            _panel = RetainedUiKit.CreateRect("GuildLauncher", canvas);
            ...
            Button button = RetainedUiKit.AddButton("OpenGuildLife", _panel, "GUILD LIFE", delegate { if (_toggle != null) _toggle(); }, Width - 20f, Height, false);
        ...
        internal void Tick(bool visible, bool open)
        {
            ...
            if (_label != null) _label.text = open ? "GUILD LIFE •" : "GUILD LIFE";
        }
